...
public class WxCom {
	...
	public static String getProviderAccessToken() {
		String access_token = "";
		String requestUrl = provider_token_url;
		// 发起POST请求获取凭证
		String jsonString = "{\"corpid\":\""+WxConstants.CORPID+"\",\"provider_secret\":\""+WxConstants.PROVIDER_SECRET+"\"}";
		JSONObject jsonObject = httpsRequest(requestUrl, "POST", jsonString);
		if (null != jsonObject) {
			try {
				access_token = jsonObject.getString("provider_access_token");
			} catch (JSONException e) {
				access_token = null;
			}
		}
